---
date_found: 2026-05-29
monitor: monitor-01
domain: privacy-data-protection
source_url: https://noyb.eu/en/conseil-detat-upholds-criteos-eu40m-gdpr-fine
source_publisher: noyb news
source_date: Fri, 13 Mar 2026 10:32:14 +0000
source_method: rss
title: Conseil d'État upholds Criteo's €40M GDPR fine
matched_keywords:
- right to erasure
- Privacy International
- CNIL
- data subject rights
- GDPR
- consent
- noyb
match_count: 7
status: candidate
entities:
- name: Commission Nationale de l'Informatique et des Libertés
  type: regulator
  jurisdiction: FR
  matched_alias: CNIL
case_numbers: []
cluster_id: cluster-2026-05-24-489228bb63
cluster_size: 7
cluster_role: sibling
cluster_canonical_slug: conseil-d-tat-upholds-criteo-s-40m-gdpr-fine
ranked_at: '2026-05-31T09:50:45+00:00'
---

# Conseil d'État upholds Criteo's €40M GDPR fine

**Source:** [noyb news](https://noyb.eu/en/conseil-detat-upholds-criteos-eu40m-gdpr-fine)
**Published:** Fri, 13 Mar 2026 10:32:14 +0000
**Matched keywords (7):** right to erasure, Privacy International, CNIL, data subject rights, GDPR, consent, noyb

## Summary excerpt

Data Subject Rights The French Data Protection Authority (CNIL) fined Criteo, a major online advertisement and tracking company in Europe, €40 million for violating the GDPR. This decision is based on complaints filed by noyb and Privacy International in December 2018. The CNIL found that the company failed to comply with data subject rights under the GDPR and could not prove that they obtained valid consent. The Conseil d’Etat rejected CRITEO's appeal and upheld the fine. Original Press Release by CNIL ( EN ) Original complaint filed in December 2018 by noyb and Privacy International Letter by the CNIL to noyb (FR) Conseil d’Etat decision 2023 news article on €40 Mio fine Criteo – prominent ad-tech player. &nbsp;The French company&nbsp;Criteo provides “behavioral retargeting” services on thousands of websites. To do so, the company places tracking cookies on websites in order to analyze browsing habits and determines which products and services a user is likely to buy. The company has data on about 370 million people in Europe. Complaint led to further investigation. In December 2018, more than 7 years ago, noyb&nbsp; and Privacy International filed complaints against Criteo for n

---

*Auto-generated candidate from `scripts/fetch_articles.py`. Review against the
persona's `interrogation-checklist.md` before promoting to a formal finding
under `output-schema.md`.*
